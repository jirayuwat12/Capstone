...
def main():
    ...
    for dir in subdir:
            ...
            for frame_time_step, frame_index in zip(time_step, frame_indexs):
                ...
                if index < len(frames) - 2:
                    if not os.path.exists(arguments_strOut_second_res_deblur_path):
                        ...
                        pstring = (
                            "Interp PSNR : " + str(round(psnr_tmp, 4)) + " Interp SSIM : " + str(round(ssim_tmp, 4))
                        )
                        logger.info(pstring)
                ...
                pstring = (
                    "blurry PSNR : "
                    + str(round(psnr_tmp, 4))
                    + " blurry SSIM : "
                    + str(round(ssim_tmp, 4))
                    + "\n"
                    + first_blurry_path
                )
                logger.info(pstring)
        ...
        pstring = (
            "Avg. folder"
            + " blurry psnr "
            + str(psnr_blurry_total.avg)
            + " deblur psnr "
            + str(psnr_interp_total.avg)
            + " interp psnr "
            + str(psnr_deblur_total.avg)
            + " blurry ssim "
            + str(ssim_blurry_total.avg)
            + " deblur ssim "
            + str(ssim_interp_total.avg)
            + " interp ssim "
            + str(ssim_deblur_total.avg)
        )
    ...
    pstring = (
        "Avg. testset "
        + " interp psnr "
        + str(psnr_deblur_total_set.avg)
        + " blurry psnr"
        + str(psnr_blurry_total_set.avg)
        + " deblur psnr"
        + str(psnr_interp_total_set.avg)
        + " interp ssim "
        + str(ssim_deblur_total_set.avg)
        + " blurry ssim"
        + str(ssim_blurry_total_set.avg)
        + " deblur ssim"
        + str(ssim_interp_total_set.avg)
    )

    logger_summary.info(pstring)

    pstring = (
        "runtime per image [s] : %.4f\n" % total_run_time.avg
        + "CPU[1] / GPU[0] : 1 \n"
