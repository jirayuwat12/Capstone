...
class ResnetGenerator(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_blocks):
            model += [
                ResnetBlock(
                    256, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias
                )
            ]  # S x 256 @ 64 x 64
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, padding_type, use_dropout, use_bias, norm_layer=nn.BatchNorm2d):
        """
          init function
        ...
        try:
            blocks = (
                padAndConv[padding_type] + [norm_layer(dim), nn.ReLU(True)] + [nn.Dropout(0.5)]
                if use_dropout
                else [] + padAndConv[padding_type] + [norm_layer(dim)]
            )
        except:
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(
        self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[], use_parallel=True
    ):
