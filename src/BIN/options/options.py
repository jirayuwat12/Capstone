...
def parse(opt_path, is_train=True):
    ...
    opt["path"]["root"] = opt["path"]["save_path"]  # todo shenwang
    if is_train:
        # experiments_root = osp.join(opt['path']['root'], 'experiments', opt['name']+str(random.randint(0,100)))# todo
        experiments_root = osp.join(opt["path"]["root"], "experiments", opt["name"])  # todo
        ...
        opt["path"]["train_images"] = osp.join(experiments_root, "train_images")
