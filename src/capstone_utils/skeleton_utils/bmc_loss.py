...
def bmc_loss(x_joints, xhat_joints):
    ...
    if torch.isnan(bl_loss) or torch.isnan(curvature_loss) or torch.isnan(phi_loss):
        ...
        with open("loss_nan_log.txt", "w") as f:
            f.write(f"{timestamp} - bl_loss: {bl_loss.item()}, curvature_loss: {curvature_loss.item()}, phi_loss: {phi_loss.item()}\n")
            f.write(f"min_bone_len: {min_bone_len.shape}, max_bone_len: {max_bone_len.shape} ,xhat_bone_lens: {xhat_bone_lens.shape} \n")
            f.write(f"min_curvatures: {min_curvatures.shape}, max_curvatures: {max_curvatures.shape} ,xhat_curvatures: {xhat_curvatures_t.shape} \n")
