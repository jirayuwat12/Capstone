...
def bmc_loss(x_joints,xhat_joints):
    ...
    x_bone_lens = torch.linalg.norm(
        x_kin_chain, ord=2, axis=-1
    )  # (N*20)
    ...
    xhat_bone_lens = torch.linalg.norm(
        xhat_kin_chain, ord=2, axis=-1
    )  # (20*N)
