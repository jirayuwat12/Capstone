...
for epoch_num in range(
    args.epoch_per_iteration, main_config["max_epochs"] + args.epoch_per_iteration, args.epoch_per_iteration
):
    ...
    if epoch_num != args.epoch_per_iteration:
        ...
        checkpoint_file = glob.glob(
            f"{main_config['wandb_save_dir']}/{prev_log_folder}/wadb/run-*/files/checkpoints/*.ckpt"
        )[0]
