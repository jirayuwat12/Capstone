...
VDO_NAME = os.path.basename(config["input_vdo_path"]).split(".")[0]
...
IS_VDO_PATH_A_FOLDER = os.path.isdir(config["input_vdo_path"])
...
os.makedirs(os.path.dirname(config["output_vdo_path"]), exist_ok=True)
## Convert from .mp4 to vdo folder if needed
if not IS_VDO_PATH_A_FOLDER:
    os.system(f"ffmpeg -i {config['input_vdo_path']} {CONVERTED_VDO_PATH}/%05d.png")
    print(f"Converted {config['input_vdo_path']} to {CONVERTED_VDO_PATH}")
    ...
    for i, file_name in enumerate(sorted(os.listdir(CONVERTED_VDO_PATH))):
        ...
        new_name = f"{CONVERTED_VDO_PATH}/{str((config['num_interpolation']+1)*(i+1)-1).zfill(5)}.png"
        os.rename(to_be_renamed, new_name)
    print(f"Renamed files in {CONVERTED_VDO_PATH} to match the interpolation")
else:
    os.system(f"cp -R {config['input_vdo_path']} {ALL_CONVERTED_VDO_PATH}")
    print(f"Copied {config['input_vdo_path']} to {CONVERTED_VDO_PATH}")
...
original_fps = cv2.VideoCapture(config["input_vdo_path"]).get(cv2.CAP_PROP_FPS)
after_fps = original_fps * (1 / config["time_step"])
os.system(
    f"ffmpeg -framerate {after_fps} -pattern_type glob -i './{DEBLURRED_VDO_PATH}/{VDO_NAME}/*.png' -c:v libx264 -pix_fmt yuv420p {config['output_vdo_path']}"
)
print(f"Converted ./{DEBLURRED_VDO_PATH}/{VDO_NAME} to {config['output_vdo_path']}")
